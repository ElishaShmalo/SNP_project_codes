...
class EDARunner:
    ...
    def plot_decomposition(self, zoom=None, save_plot=False, file_path=None):
        '''
        ...
        '''
        # This function was generated largly with the help of chatGPT
        amount_to_show = zoom
        if zoom == None:
          amount_to_show = len(self.df[self.col_names[0]])

        start_index = self.df[self.col_names[0]][0]  # Replace with your desired start index
        end_index = self.df[self.col_names[0]][amount_to_show-1]    # Replace with your desired end index

        # Set the datetime column as the index
        decomp_df = self.df.set_index(self.col_names[0])

        # Decompose the time series data into its components: observed, trend, seasonal, and residual
        decomposition = seasonal_decompose(decomp_df[self.col_names[1]], model='additive', period=365)
        ...
        # Define the start and end indices for the subset you want to plot
        start_index = self.df[self.col_names[0]][0]
        end_index = self.df[self.col_names[0]][amount_to_show-1]

        # Plot the observed data with preserved datetime index
        observed_subset = decomposition.observed[start_index:end_index]
        observed_subset.plot(ax=ax1, color=self.colors[0])
        ax1.set_ylabel('Observed')

        # Plot the trend component
        trend_subset = decomposition.trend[start_index:end_index]
        trend_subset.plot(ax=ax2, color=self.colors[2])
        ax2.set_ylabel('Trend')

        # Plot the seasonal component
        seasonal_subset = decomposition.seasonal[start_index:end_index]
        seasonal_subset.plot(ax=ax3, color=self.colors[1])
        ax3.set_ylabel('Seasonal')

        # Plot the residual component
        residual_subset = decomposition.resid[start_index:end_index]
        residual_subset.plot(ax=ax4, color=self.colors[3])
